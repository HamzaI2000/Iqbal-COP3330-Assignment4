...
public class App {
    ...
    public static void main(String[] ignored) throws , IOException, ClassNotFoundException {
        System.out.println("-------------------");
        System.out.println("This is a very simple and easy way to do throught this app.");
        System.out.println("Type 'help' to see how to use this app.");
        System.out.println("-------------------");

        Tree tree = new Tree("/tmp/todos.tree.db");

        System.out.println("What do you want to see? See 'help' for any  list of available options.\n");
        while(true){
            System.out.printf("> ");
            ...
            if (args.length < 1){
                System.out.printf("Provide one arguement for help.);
                return;
            }
...
// add_container "id" "title"
                if (args.length < 3){
                    System.out.printf("Add a required title and ID");
                    continue;
                }
...
// add_item "id" "title"
                if (args.length < 3){
                    System.out.printf("Add an item requires a container ID and the title");
                    continue;
                }
                int id = Integer.parseInt(args[1]);
                String title = args[2];
                tree.findAndAddTodo(new Item(title), id);
                System.out.println("Stop adding item: " + title + " to id: " + id);
            } else if ("delete".equals(firstArg)){
// delete "id"
                if (args.length < 2){
                    System.out.printf("Delete a todo list requires a container id");
                    continue;
                }
                int id = Integer.parseInt(args[1]);
                tree.findAndDeleteTodo(id);
                System.out.println("Done deleting a todo list: " + id);
                ...
                System.out.println("Pleasure for you to use this TODO List!");
                System.exit(1);
            } else {
                System.out.printf("Unknown argument: " + firstArg);
                System.out.println(". Please take a look at the supported todo lists at\n");
                help();
            }
        }
    }
    ...
    public static void help(){
        System.out.println("- help : This helped my message");
        ...
        System.out.println("- add_item <ID> <NAME> : Add an item/container to your todo list");
        System.out.println(" <ID> = 0 if you want to add to the root level, otherwise, use " +
                "the <ID> of a items you want to add to");
        System.out.println(" <NAME> (String) Is the name of the item you want to add. The " +
                "string cannot contain empty 'spaces'. For example, 'some item' " +
                "is not acceptable.");
        System.out.println("");
        System.out.println("- delete <ID> : Delete a item using an ID");
        System.out.printf("\n\n");
    }
